...
source "$CONFIG_FILE"
if [[ ! -z "$FONT_SIZE" ]]; then
osascript -e "tell application \"Terminal\" to set font size of window 1 to $FONT_SIZE"
else
osascript -e 'tell application "Terminal" to set font size of window 1 to 15'
fi
else
...
CURRENT_EDITION="Mac OS" 
C_ORANGE2=$(tput setab 214)
F_BOLD=$(tput bold)
F_INVERT=$(tput rev)
C_WHITE=$(tput setaf 15)
C_GREY100=$(tput setaf 231)
C_GREY3=$(tput setaf 232)
C_GREY0=$(tput setab 0)
C_ORANGE1=$(tput setab 214)
NO_FORMAT=$(tput sgr0)
C_RED1=$(tput setaf 1)
...
local text_width=${#text}
C_SEAGREEN2="\033[38;5;83m"
echo -en "${C_RED1}${C_BLACK}"
echo -n "${text}"
printf "%$((total_width - text_width - 17))s${NO_FORMAT}\n"
}
printf '\033c'
clear && printf '\e[3J'
while true; do
...
echo -en "${C_ORANGE2}${C_BLACK}"
echo -n "2. Auto Update: "
center_text5_5 "False"
fi
center_text2 "3. Set Font Size (Standard: 15)"
center_text2 "4. Return to main menu"
center_text1 "Enter your choice:"
read SETTINGS_CHOICE
case $SETTINGS_CHOICE in
1)
echo "Checking for updates..."
LATEST_VERSION=$(curl -s "https://raw.githubusercontent.com/administrati0n/http-master/master/mac/ASV.txt")  # получаем последнюю версию
...
curl -s -o "$TMP_FILE" "https://raw.githubusercontent.com/administrati0n/http-master/main/mac/AS-http-server.sh"
...
3)
read -p "Enter desired font size (default is 15): " font_size
if [[ "$font_size" =~ ^[0-9]+$ ]]; then
osascript -e "tell application \"Terminal\" to set font size of window 1 to $font_size"
echo "Font size changed to $font_size!"
update_config_value "FONT_SIZE" "$font_size"
else
...
case $choice in
1)
dest_path="$HOME/Documents"
;;
2)
dest_path="$HOME/Downloads"
;;
3)
