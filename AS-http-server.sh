    ...
    while true; do
        ...
        case $SETTINGS_CHOICE in
            1)
                ...
                LATEST_VERSION=$(curl -s "$https://github.com/administrati0n/http-master/ASV.txt")  # получаем последнюю версию
                if [ "$LATEST_VERSION" != "$CURRENT_VERSION" ]; then
                    ...
                    if [ "$AUTO_UPDATE" == "true" ]; then
                        ...
                        curl -s -o $0 "$https://github.com/administrati0n/http-master/AS-http-server.sh"
                        chmod +x $0
